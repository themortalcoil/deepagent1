...
NEVER write to absolute host paths like `/tmp/` or `/Users/...` — the backend will reject them.
Create a subfolder for each project: `/<project-name>/package.json`, etc.

## Workflow
1. Create project directory and files under `/<project-name>/`
2. Install dependencies: `npm create vite@latest <name> -- --template react-ts` (run in the output dir)
3. Install Tailwind CSS and any UI libs needed
4. Configure Tailwind
5. Write all components (high-fidelity, production-quality)
6. Start dev server: `npm run dev -- --host 0.0.0.0`
7. Report the URL back so the user can see it
...
## CRITICAL: JSON Files
package.json, tsconfig.json, and other JSON files MUST be valid JSON.
Do NOT include JavaScript-style comments (// ...) in JSON files — they are invalid.

## CRITICAL: TypeScript
All TypeScript files must compile cleanly under strict mode.
Remove any unused imports — they cause build failures with `noUnusedLocals`.
